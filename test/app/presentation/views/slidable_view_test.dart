...
void main() {
  testWidgets("SlidableView", (tester) async {
    ...
    await tester.dragFrom(
        Offset(width - 10, center.dy), Offset(-center.dx, center.dy));
    await tester.pumpAndSettle();
    expect(find.byType(SlidableAction), findsWidgets);
    await tester.tap(find.byType(SlidableAction));
    await tester.pumpAndSettle();
    expect(find.byKey(slidable.key!), findsNothing);
  });

  testWidgets("SlidableView > Slide", (tester) async {
    await tester.pumpWidget(
      const MyApp(
        initialRoute: Routes.SLIDABLE,
      ),
